...
function CreateGigLabelsForm() {
  const [appLabels, setAppLabels] = useState(labels);
  const [customLabels, setCustomLabels] = useState<string[]>([]);

  const { setStep } = useCreateGig();

  const selectedLabels = [
    ...appLabels.filter((l) => l.isSelected),
    ...customLabels,
  ];
  ...
  function addCustomLabel(label: string) {
    ...
    const customLabelExists = customLabels.some(
      (l) => l.toLowerCase() === label.toLowerCase()
    );
    if (customLabelExists) return;
  ...
  return (
    <form onSubmit={() => console.log("Form submitted")}>
      ...
      <StepNavigation isValid={false} nextStepName="Location" />
    </form>
  );
}
